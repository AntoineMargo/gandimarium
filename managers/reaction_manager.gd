...
func elicit_reaction(reaction_event, creature) -> void:
	creature.handle_event(reaction_event)
...
#
	#for reaction in creature.data.reactions:
		...
		#for trigger in reaction.triggers:
				...
				#var activity = reaction.activity.query_current_activity(creature)
...
func handle_event(reaction_event: ReactionEvent) -> void:
	var main_creature: Creature = reaction_event.context.user
	main_creature.move_linked_area_conditions(reaction_event)
