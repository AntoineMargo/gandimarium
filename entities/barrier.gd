...
func take_damage(damage: int, resistance: Enums.Resistance):
	...
	if final_damage <= 0:
		...
		if durability_type == Enums.BarrierDurabilityType.CHARGES:
			durability -= 1
			SignalBus.message.emit("%s absorbed %d damage for %s!" % [parent_condition.name, final_damage, original_target.data.name])
		else:
			durability -= final_damage
			var remaining_damage: int = abs(final_damage)
			if durability <= 0:
				SignalBus.message.emit("%s took %d damage instead of %s!" % [parent_condition.name, (final_damage - remaining_damage), original_target.data.name])
				if durability_type == Enums.BarrierDurabilityType.HP_NON_BLOCKING:
					original_target.take_damage(remaining_damage, resistance)
			else:
				SignalBus.message.emit("%s took %d damage instead of %s!" % [parent_condition.name, final_damage, original_target.data.name])
	if durability <= 0:
